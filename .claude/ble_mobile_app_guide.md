...
**No countdown ETA on this dialog.** `savedSessionFileSize` is only captured from the `STATE_HAS_SAVED_SESSION` Status payload (FW commit `3990cf22`). A typical BLE drop/reconnect to a still-running device sends `STATE_RUNNING` directly — Scenario A in §6a — so the app never sees a file_size, and an ETA built on that field would be perpetually unavailable. The dialog therefore only shows the static "syncing in progress" description; adding an ETA here requires firmware to extend the 18-byte `STATE_RUNNING` payload with a `file_size_u64_LE` suffix (or expose a query opcode).
