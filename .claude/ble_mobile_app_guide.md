...
**Countdown ETA:** when the firmware reports a non-zero `savedSessionFileSize` on `STATE_HAS_SAVED_SESSION` (FW commit `3990cf22`), the dialog renders a per-second countdown in its description. The total estimate is computed once at dialog open via `V2BleSyncOrchestrator.estimateSyncSeconds(savedSessionFileSize)` (the same throughput model the sync-before-new dialog uses). Elapsed time is measured from `AirBeamMiniV2StateRepository.activeSyncStartMs` — set on the first Sync (`0006`) chunk after `parseStatus()` clears the timestamp on every fresh `STATE_HAS_SAVED_SESSION`, so the anchor is the start of this drain (not a prior reconnect's). If no chunks have arrived yet when the dialog opens, the elapsed-time anchor falls back to the dialog-open wall clock. Older firmware (file_size 0) skips the ETA and shows the size-less description.
