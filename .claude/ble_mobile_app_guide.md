...
`SyncAndFinishV2SessionDialog` drives the **BLE manual-sync** flow — the same `V2BleSyncOrchestrator` path used by `SyncBeforeNewV2SessionDialog`. It does **not** wait for the passive Active Sync stream. The flow:

1. Renders header + description. When `AirBeamMiniV2StateRepository.savedSessionFileSize > 0` (HasSavedSession payload, FW commit `3990cf22`), prepends an ETA built from `V2BleSyncOrchestrator.estimateSyncSeconds(fileSize)`. If the device was reached via `STATE_RUNNING` (no file_size suffix), the ETA-less description is rendered.
2. Primary button "Sync & Finish Session" calls `v2StateRepository.startSync(keepConnectedAfter = true)`. The orchestrator writes `StartBleSync (0x16)` — the firmware stops the running session in its Stop handler, so no new mobile measurements arrive after this point. The drain streams over the Sync characteristic, the orchestrator inserts records via `V2MobileMeasurementsInserter`, and `markMobileFinished()` marks the session FINISHED in the local DB.
3. Secondary button "Finish Without Syncing" skips the sync entirely and routes directly to `onFinishMobileSessionConfirmed(session)`. `AirBeamConnector.onMessageEvent(StopRecordingEvent)` then runs `discardSession()` (writes `0x11 DiscardSession`) followed by `disconnect()`, wiping any remaining on-device measurements.
4. On sync success, a "Done" state is rendered; the user taps to finish the session (post `StopRecordingEvent`, which also runs the standard `discardSession()` — by now firmware storage is already cleared so the Discard is effectively a no-op).
5. On sync failure (write error, Nack 0x06 SyncFailed, or timeout), a "Sync failed" state is rendered with a "Continue" button that finishes the session via `StopRecordingEvent` — the same discard path as the skip route.
