...
class AddNoteBottomSheet(
    ...
    private fun addNote(mSession: Session) {
        ...
        val date = Date()
        val note = Note(date, noteText, mSession.streams[0].measurements.last().latitude, mSession.streams[0].measurements.last().longitude, mSession.notes.size) // todo: add "photoPath" later on
